#ifndef CONTROLLER_H
#define CONTROLLER_H
#include "../src/motionCommandType.h"
#include "../src/motionInterfaceBuffer.h"

#include "../src/motionParameterType.h"
#include "../src/motionConfig.h"
#include "../src/motionFeedback.h"
#include "../src/posemath.h"
...
//6点法工具坐标系自动校准，jp: 6个点的数组的首地址，pointNumber:暂时只支持6点.
//out: 计算出的结果。
extern int CTRL_ToolCalibration(JointPoint *jp,int pointNumber,ToolCoordianteInformation *out);


//scara的运动学正解，jp输入的角度值。pos输出的位置值。
extern int CTRL_SCARA_KineticForward(JointPoint *jp,PmHomogeneous *pos);

//scara的运动学反解，jp输入的角度值。pos输出的位置值。

extern int CTRL_SCARA_KineticInverse(PmHomogeneous *pos,JointPoint *jp);

//scara的运动学正解，输出为X/Y/Z/A/B/C，

extern int CTRL_SCARA_KineticForwardInABC(JointPoint *jp,PointPose *point);

//scara的运动学反解，输入为X/Y/Z/A/B/C

extern int CTRL_SCARA_KineticInverseInABC(PointPose*point,JointPoint *jp);
