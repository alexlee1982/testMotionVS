...
/*
	new function to calculate the circle centre
	input : three points on the circle
	output : center point of the circle
...
PmCartesian threePointCircle(PmCartesian A, PmCartesian B, PmCartesian C)
{
	...
	pmCartScalMult(ex, xC, &vx);
	//计算 vy = v - vx;
	pmCartCartSub(v, vx, &vy);
	...
	//计算center = A + 1/2 xB*ex + (-xB (xB - xC) xC + xB yC^2)/(2 xB yC)*ey
